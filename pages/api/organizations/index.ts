...
import { createOrganizationSchema, validateWithSchema } from '@/lib/zod';
...
const handlePOST = async (req: NextApiRequest, res: NextApiResponse) => {
  const { name } = validateWithSchema(createOrganizationSchema, req.body);

  const user = await getCurrentUser(req, res);
  const slug = slugify(name);

  if (await isOrganizationExists(slug)) {
    throw new ApiError(400, 'An organization with the slug already exists.');
  }
